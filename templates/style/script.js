...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if(document.getElementById("fecha-ida")) flatpickr("#fecha-ida", configCalendario);
    if(document.getElementById("fecha-vuelta")) flatpickr("#fecha-vuelta", configCalendario);
...
// 1. SANEAMIENTO INICIAL: Forzamos limpieza de memoria corrupta
let savedAdults = parseInt(localStorage.getItem('pax_adults'));
let savedKids = parseInt(localStorage.getItem('pax_kids'));

// Si lo que hay guardado no es un número o es mayor de 10, reseteamos a 1
if (isNaN(savedAdults) || savedAdults > 10 || savedAdults < 1) savedAdults = 1;
if (isNaN(savedKids) || savedKids > 10 || savedKids < 0) savedKids = 0;
...
function initPasajeros() {
    console.log("🚀 Sistema Limpio: Iniciando con " + paxState.adults + " Adulto(s)");
    ...
    const sincronizarTodo = () => {
        ...
        // Guardamos valores limpios
        localStorage.setItem('pax_adults', a);
        localStorage.setItem('pax_kids', k);

        // Actualizar los numeritos en el menú (+ y -)
        if(document.getElementById('pax-adults-val')) document.getElementById('pax-adults-val').innerText = a;
        if(document.getElementById('pax-kids-val')) document.getElementById('pax-kids-val').innerText = k;
        
        // Actualizar el texto del botón principal (el que ve el cliente)
        const display = document.getElementById('pasajeros-display');
        if(display) {
            display.innerText = `${a} Adulto${a > 1 ? 's' : ''}${k > 0 ? ' + ' + k + ' Niñ.' : ''}`;
        }

        // Generar formularios de datos (solo si existe el contenedor)
        ...
        document.querySelectorAll('.precio-final').forEach(el => {
            ...
            const final = (base * total) + (Number(paxState.seguro) * total);
            el.innerText = final.toLocaleString('es-ES') + '€';
        });
    };
    ...
    document.addEventListener('click', (e) => {
        ...
        if (btn) {
            ...
            if (type === 'adults') {
                const result = Number(paxState.adults) + step;
                if (result >= 1 && result <= 10) paxState.adults = result;
            } else if (type === 'kids') {
                const result = Number(paxState.kids) + step;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Base de datos de contenido personalizado para Andrea
  const infoData = {
        msc: {
            title: "MSC <span class='gold'>Yacht Club</span>",
            img: "images/msccru.jpg",
            desc: "Un yate dentro de un barco. Privacidad, mayordomo 24h y áreas exclusivas para quienes buscan el máximo confort.",
            features: ["Check-in prioritario", "Bebidas premium incluidas", "Solárium y piscina privada"]
        },
        royal: {
            title: "Royal <span class='gold'>Caribbean Adventure</span>",
            img: "images/royalcru.jpg",
            desc: "La flota más innovadora del mundo con simuladores de surf, espectáculos de Broadway y tecnología de vanguardia.",
            features: ["Isla privada Perfect Day at CocoCay", "Gastronomía internacional", "Ocio para todas las edades"]
        },
        mediterraneo: {
            title: "Tesoros del <span class='gold'>Mediterráneo</span>",
            img: "https://images.unsplash.com/photo-1523906834658-6e24ef2386f9?q=80&w=1200",
            desc: "Rutas exclusivas por las islas griegas, la costa italiana y rincones secretos diseñados por Andrea.",
            features: ["Guías locales expertos", "Excursiones privadas", "Hoteles boutique en cada puerto"]
        },
        // --- AQUÍ LOS NUEVOS ---
        fiordos: {
            title: "Magia de los <span class='gold'>Fiordos</span>",
            img: "https://images.unsplash.com/photo-1516939884455-1445c8652f83?q=80&w=1200",
            desc: "Navega entre montañas majestuosas y cascadas infinitas en una de las rutas más visuales del planeta.",
            features: ["Navegación por glaciares", "Pueblos con encanto nórdico", "Safaris fotográficos de naturaleza"]
        },
        caribe: {
            title: "Paraíso <span class='gold'>Caribeño</span>",
            img: "https://images.unsplash.com/photo-1548574505-5e239809ee19?q=80&w=1200",
            desc: "Desconecta en playas de arena blanca y aguas turquesas con itinerarios que combinan relax y cultura local.",
            features: ["Islas exclusivas", "Buceo en arrecifes de coral", "Ritmos y sabores tropicales"]
        }
    };
    ...
    function openPanel(type) {
        ...
        content.innerHTML = `
            <div class="panel-header-img" style="background-image: url('${data.img}')"></div>
            <div class="panel-body">
                <h2>${data.title}</h2>
                <p class="panel-desc">${data.desc}</p>
                <ul class="panel-list">
                    ${data.features.map(f => `<li><i class="fas fa-anchor"></i> ${f}</li>`).join('')}
                </ul>
                <a href="contacto.html" class="btn-presupuesto-panel">SOLICITAR ITINERARIO PERSONALIZADO</a>
            </div>
        `;
    ...
    // Listener para los botones de las Navieras (MSC y Royal)
document.querySelectorAll('.btn-luxe').forEach((btn, index) => {
    btn.addEventListener('click', function(e) {
        e.preventDefault(); // Detiene el salto al principio
        e.stopPropagation(); // Evita que el evento suba a otros elementos
        const type = index === 0 ? 'msc' : 'royal';
        openPanel(type);
    });
});

// Listener para los botones de las Regiones (Mediterráneo, Fiordos, Caribe)
document.querySelectorAll('.explore-btn-premium').forEach((btn, index) => {
    btn.addEventListener('click', function(e) {
        e.preventDefault(); // Detiene el salto al principio
        e.stopPropagation();
        const regions = ['mediterraneo', 'fiordos', 'caribe'];
        openPanel(regions[index]);
    });
});
...
function openPanel(type) {
    // ... resto del código de antes ...
    sidePanel.classList.add('active');
    overlay.classList.add('active');
    document.body.style.overflow = 'hidden'; // Bloquea el scroll del fondo
}

// En el evento de cerrar (clic en overlay o botón cerrar)
function closePanel() {
    sidePanel.classList.remove('active');
    overlay.classList.remove('active');
